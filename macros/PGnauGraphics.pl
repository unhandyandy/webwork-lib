...
sub Plot{
  ...
  while (scalar @options > 0){
    ...
    if ($val eq 'single'){
      ...
      $tex = shift @options;
  } elsif ( $val eq 'extra_html_tags') {
      $extraTags .= shift @options;
  } else {
      shift @options;
    }
  }
  ...
  $plot = image(insertGraph($pic), width=>$width, height=>$height, tex_size=>$tex, extra_html_tags=>$extraTags);
};
