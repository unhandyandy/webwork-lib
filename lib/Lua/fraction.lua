...
function fraction.unm( fr )
   local res = fr:clone()
   res.sign = -res.sign
   return res
end

function fraction.lt ( a, b )
...
function fraction.add( f1, f2 )
   if fraction.isfraction( f1 ) then
	 ...
	 return addFractions( f1, newfr )
      ...
      return fraction.add( f2, f1 )
   end
end

function fraction.sub( f1, f2 )
...
function fraction.div( f1, f2 )
   return fraction.mul( f1, fraction.invert( f2 ) )
end

function fraction.eq( f1, f2 )
   if not fraction.isfraction( f1 ) then
      if type( f1 ) == "number" then
	 return fraction.eq( fraction.fromDecimal( f1 ), f2 )
      else
	 return false
      end
   elseif not fraction.isfraction( f2 ) then
      if type( f1 ) == "number" then
	 return fraction.eq( f1, fraction.fromDecimal( f2 ) )
      else
	 return false
      end
   else
