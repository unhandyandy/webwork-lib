...
matrix.isMathType = true
matrix.ismatrix = true

local zerofrac = fraction.new( 0, 1 )
...
function matrix.add(a,b)
   assert(a.ismatrix and b.ismatrix, "Add: wrong argument types (<matrix> expected)")
   assert( a.dim == b.dim, "Add: wrong argument lengths (same dim expected)")
   local sum = vector.zero( a.dim[2] )
   for i = 1, a.dim[1] do
      sum[i] = vector.add( a[i], b[i] )
   end
...
function matrix.sub(a,b)
   ...
   local sum = vector.zero( a.dim[2] )
   for i = 1, a.dim[1] do
      sum[i] = vector.sub( a[i], b[i] )
   end
   return matrix.new( sum )
end


function matrix.scalarMult( s, m )
   local res = {}
   for i = 1, m.dim[1] do
      table.insert( res, vector.mul( s, m[i] ) )
   end
   return matrix.new( res )
end
...
local function matProd( m1, m2 )
   ...
   local res = {}
   for i = 1, m1.dim[1] do
      local newrow = vector.zero( m2.dim[2] )
      for j = 1, m1.dim[2] do
         local mult = vector.scalarMult( m1[i][j], m2[j] )
	 newrow = vector.add( newrow, mult )
      end
...
function matrix.mul(a,b)
   if ( type(a) == "number" or a.isfraction ) and b.ismatrix then
      return matrix.scalarMult( a, b )
   elseif ( type(b) == "number" or b.isfraction ) and a.ismatrix then
      return matrix.scalarMult( b, a )
   else
      assert(a.ismatrix and b.ismatrix, "Mul: wrong argument types (<matrix> or <number> expected)")
      return matProd( a, b )
   end
end
...
function matrix.div(a,b)
        assert(a.ismatrix and type(b) == "number", "wrong argument types (expected <matrix> / <number>)")
        return scalarMultLua( 1 / b , a )
end

function matrix.eq(a,b)
...
function matrix.random( r, c, max, sgnF )
   ...
   local lst = {}
   for i = 1, r do
      table.insert( lst, vector.random( c, max, sgnF ) )
   end
...
function matrix.pow( m, p, tot )
   tot = tot or matrix.identity( m.dim[1] )
   if p == 0 then
      return tot
   elseif p % 2 == 0 then
      return matrix.pow( matrix.mul( m, m ), p / 2, tot )
   else 
      return matrix.pow( matrix.mul( m, m ), (p - 1) / 2, matrix.mul( m, tot ) )
   end
end 
...
function matrix:determinant()
   ...
   mins = vector.new( mins )
   return vector.mul( col1, mins )
end
...
function matrix:inverse()
   ...
   --print( '...done \n' )
   return fraction.one() / matrix.mul( det, res )
end 
...
function matrix:isregular( n )
   ...
   if self:hasZeros() then
	 ...
	 return ( matrix.mul( self, self ) ):isregular( n + 1 )
      end 
   else 
      return true
   end 
...
function matrix.new( lst )
   ...
   local res = copy( matrix )
   for _, v in ipairs( lst ) do
      table.insert( res, vector.new( v ) )
   end
   res.dim = vector.new( { #lst, #lst[1] } )
   return res
end
