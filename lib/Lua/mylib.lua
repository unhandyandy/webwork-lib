...
function mklatex( tmpl, sub )
   return string.format( tmpl, unpack( sub ) )
end
...
function distinctSummands( n, sum )
   local p = math.floor( sum / n + 0.5 )
   local lst = pack( distinctRands( n, 0, 2 * p ) )
...
function flatten( tab )
   if type( tab ) == 'table' and not mathTypeQ( tab ) then
      return listJoin( unpack( map( tab, flatten ) ) )
   else
      return { tab }
   end
end
...
function signIter( d, frm )
   ...
   local function res()
      i = i + 1
      if i < 2^d then 
	 return unpack( numToSigns( i, d, frm ) )
      end
   end 
   return res
end 
